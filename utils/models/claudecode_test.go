...
func TestClaudeCodeBuildArgsAgentic(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			allowedPaths:    []string{"./"},
			tools:           nil,
			contains:        []string{"-p", "--bare", "--add-dir", "--dangerously-skip-permissions", "--allowedTools", "Read,Write,Edit,Glob,Grep,Bash", "--output-format", "json", "--model", "claude-sonnet-4-5-20250929", "explore"},
		...
		},
		{
			...
			contains:        []string{"-p", "--bare", "--add-dir", "/tmp", "--dangerously-skip-permissions", "--allowedTools", "Read,Bash", "--output-format", "json", "test"},
		...
		},
		{
			...
			tools:           nil,
			contains:        []string{"-p", "--bare", "--add-dir", "--dangerously-skip-permissions", "--allowedTools", "Read,Write,Edit,Glob,Grep,Bash", "--output-format", "json", "analyze"},
			notContains:     []string{"--print"},
			checkAbsPaths:   true,
			expectedPathCnt: 2,
		},
		{
			...
			allowedPaths:    []string{},
			tools:           nil,
			contains:        []string{"-p", "--bare", "--output-format", "json", "query"},
