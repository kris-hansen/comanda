...
func TestClaudeCodeBuildArgsAgentic(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			allowedPaths:    []string{"./"},
			tools:           nil,
			contains:        []string{"-p", "--bare", "--add-dir", "--dangerously-skip-permissions", "--output-format", "json", "--model", "claude-sonnet-4-5-20250929", "explore"},
			notContains:     []string{"--print", "--allowedTools"},
			checkAbsPaths:   true,
			expectedPathCnt: 1,
		},
		{
		...
		},
		{
			...
			tools:           nil,
			contains:        []string{"-p", "--bare", "--add-dir", "--dangerously-skip-permissions", "--output-format", "json", "analyze"},
			notContains:     []string{"--print", "--allowedTools"},
			checkAbsPaths:   true,
			expectedPathCnt: 2,
		},
