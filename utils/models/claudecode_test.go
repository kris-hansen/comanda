...
func TestClaudeCodeBuildArgsAgentic(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			allowedPaths:    []string{"./"},
			tools:           nil,
			contains:        []string{"--add-dir", "--dangerously-skip-permissions", "--model", "claude-sonnet-4-5-20250929", "explore"},
			notContains:     []string{"--print", "-p"},
			checkAbsPaths:   true,
			expectedPathCnt: 1,
		},
		{
			...
			contains:        []string{"--add-dir", "/tmp", "--dangerously-skip-permissions", "--tools", "Read,Bash", "test"},
			notContains:     []string{"--print", "--model", "-p"},
		...
		},
		{
			...
			tools:           nil,
			contains:        []string{"--add-dir", "--dangerously-skip-permissions", "analyze"},
			notContains:     []string{"--print", "-p"},
			checkAbsPaths:   true,
			expectedPathCnt: 2,
		},
		{
			...
			notContains:     []string{"--print", "--add-dir", "--dangerously-skip-permissions", "-p"},
			checkAbsPaths:   false,
			expectedPathCnt: 0,
		},
	}
